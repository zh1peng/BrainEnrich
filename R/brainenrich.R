...
#'
#' @param brain_data A data frame of brain data with regions as rows. The row names (i.e., region names) must be consistent with those in gene_data.
#' @param gene_data A data frame of gene expression data with regions as rows and genes as columns. The row names (i.e., region names) must be consistent with those in brain_data.
...
#'                   and 'pls1w'. If a custom function that takes (gene_data, brain_data) as input is provided, the function will use the custom correlation method, and `cor_method` will be set to 'custom'.
...
#'                     'sign_test', and 'rank_sum'. If a custom function that takes (geneList, geneSet) as input is provided, the function will use the custom aggregation method, and `aggre_method` will be set to 'custom'.
...
#' @param perm_id A matrix of permutation indices for the 'spin_brain' null model. Default is NULL. Either perm_id or coord.l/coord.r must be provided if choosing the 'spin_brain' mode.
#' @param coord.l A matrix of coordinates for the left hemisphere used in the 'spin_brain' null model. Default is NULL. Can be NULL if coord.r or perm_id is provided.
#' @param coord.r A matrix of coordinates for the right hemisphere used in the 'spin_brain' null model. Default is NULL. Can be NULL if coord.l or perm_id is provided.
#' @param seed An integer specifying the seed for reproducibility when using the 'spin_brain' model. Default is NULL.
#' @param n_cores An integer specifying the number of cores to use. Default is 0 (use all available cores minus one).
