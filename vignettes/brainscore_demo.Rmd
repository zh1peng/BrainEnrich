---
title: "Enrichment analysis of individual-level IDPs (brainscore)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Enrichment analysis of individual-level IDPs (brainscore)}
...
This would cover a large proportion of the scenarios where gsScores will be used. The predictor can be either a single numeric variable or a single two-level factor variable. The covariates can be a data frame with multiple numeric variables or factors. The function will first get the emprical statistics (i.e., t-value corresponding to the predictor) and then test that against the null model (e.g., spin_brain) to get the p-value.
