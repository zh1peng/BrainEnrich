...
```
The gsScore.raw is a list of molecular terms. For each molecular term, you can find individual gene set scores and use them for downstream analyses (e.g., correlation analysis, machine learning, clustering etc.). By default, empirical individual-level GS scores also include KS normality diagnostics:
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
head(attr(gsScore.raw, "normality"))
```
These diagnostics are intended to guide downstream model choice and are not used as automatic exclusion criteria.
