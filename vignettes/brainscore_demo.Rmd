...
#### Normality diagnostics for empirical GS scores
By default, empirical individual-level GS scores include normality diagnostics based on the Kolmogorov-Smirnov (KS) test. These diagnostics are stored as an attribute of the `brainscore` output:
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
normality_res <- attr(gsScore.raw, "normality")
head(normality_res)
```
The main columns are `normality_ks_D`, `normality_ks_p`, `normality_ks_padj`, and `normality_flag`. The flag is based on the adjusted KS p-value and is intended to identify terms whose individual GS score distributions may be unsuitable for parametric downstream models. These diagnostics are intended to guide downstream model choice and are not used as automatic exclusion criteria.

The same diagnostics can also be run directly on an existing empirical GS score object:
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
normality_res2 <- brainscore.normality(gsScore.raw)
head(normality_res2)
```
...
For the built-in linear regression workflow, `brainscore.lm_test()` supports `aggre_method = "mean"`, `"median"`, `"ks_pos_neg_sum"`, or a custom aggregation function. Other aggregation methods remain available through `brainscore()` for customized downstream analyses.
...
```
The linear regression result table also reports the same individual-level GS score normality diagnostics for each term:
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
res.df %>%
  dplyr::select(ID, starts_with("normality_")) %>%
  head()
```
