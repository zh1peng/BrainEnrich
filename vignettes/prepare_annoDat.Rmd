...
# saveRDS(annoData, file = "extdata/geneSets/KEGG.rds", compress = "xz")
packageVersion("clusterProfiler")
```
...
# saveRDS(annoData, file = "extdata/geneSets/WikiPathways.rds", compress = "xz")
packageVersion("clusterProfiler")
```
...
# saveRDS(annoData, file = "extdata/geneSets/Reactome.rds", compress = "xz")
annoData <- readRDS()
...
annoData <- build_Anno(TERM2GENE, TERM2NAME)
# saveRDS(annoData, file = "extdata/geneSets/CellTypes_Seidlitz2020.rds", compress = "xz")
```
...
TERM2NAME <- TERM2GENE %>%
  ...
  dplyr::select(term, description)
annoData <- build_Anno(TERM2GENE, TERM2NAME)
# saveRDS(annoData, file = "extdata/geneSets/CellTypes_Lake2018.rds", compress = "xz")
```
...
TERM2NAME <- TERM2GENE %>%
  ...
  dplyr::select(term, description)
annoData <- build_Anno(TERM2GENE, TERM2NAME)
# saveRDS(annoData, file = "extdata/geneSets/CellTypes_Martins2021.rds", compress = "xz")
```
...
library(MeSHDbi)
ah <- AnnotationHub(localHub=TRUE) # localHub=TRUE if you have downloaded the data
hsa <- query(ah, c("MeSHDb", "Homo sapiens"))
file_hsa <- hsa[[1]]
db <- MeSHDbi::MeSHDb(file_hsa)
get_MeSH_data <- getFromNamespace('get_MeSH_data','meshes')
MeSH_DATA <- get_MeSH_data(db, 'gendoo', 'F')
saveRDS(MeSH_DATA, file = "inst/extdata/geneSets/test.rds", compress = "xz")
testList=get_geneSetList(MeSH_DATA)
str(testList)
get_termDescription("D000126", MeSH_DATA)
```


# render me
```{r include=F, eval=FALSE}
knitr::knit("vignettes/prepare_annoDat.Rmd")
```
