...
# Install BrainEnrichData first so compute nodes share the same local resources
...
For shared HPC nodes, prefer setting `n_cores` explicitly instead of relying on auto-detection.
In the Rscript, we set `n_cores=63` to use 63 cores, which is one less than the 64-core node maximum.
```{bash,eval=FALSE}
#!/bin/bash
