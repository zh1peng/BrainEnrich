...
If the precomputed null scores will later be used with `brainscore.lm_test()`, use only `mean`, `median`, and `ks_pos_neg_sum` for `aggre_methods`.
```{bash,eval=FALSE}
#!/bin/bash
#SBATCH --job-name=gsScore       # Job name
...
null_models <- c("resample_gene", "spin_brain")
gs_types <- c("SynGO", "GO_MF")
aggre_methods <- c("mean", "median", "ks_pos_neg_sum")
