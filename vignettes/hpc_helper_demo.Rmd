---
title: "How to precompute null gene set scores and do simulation/analysis on HPC with slurm"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{How to precompute null gene set scores and do simulation on HPC with slurm}
...
Here, we will prepare a script for distributed computing environments, allowing large-scale permutation tests to be split across multiple compute nodes. 
```{r,eval=FALSE}
# brainscore_simulate_vacc.R
# Rscript brainscore_simulate_vacc.R <job_id> <n_iter_per_job> <iter_total> <cor_method> <aggre_method> <sim_type> <gs_type> <minGSSize> <maxGSSize> <gsScoreList.null.rds file>
# Get command-line arguments
args <- commandArgs(trailingOnly = TRUE)
job_id <- as.integer(args[1])
n_iter_per_job <- as.integer(args[2])
...
job_splitter(
  ...
  gsScoreList.null = gsScoreList.null # precomputed null gene set scores as specified by the user
)
```
...
#SBATCH --output=sim.out  # Standard output log
#SBATCH --error=sim.err   # Standard error log
#SBATCH --time=24:00:00             # Time limit hrs:min:sec
#SBATCH --nodes=1                    # Number of nodes
#SBATCH --ntasks=1                   # Number of tasks
#SBATCH --mem=16G                    # Memory per node
...
for gs_type in "${gs_types[@]}"; do
  for sim_type in "${sim_types[@]}"; do
    ...
    if [[ "$sim_type" == "randomize_pred" ]]; then
      ...
      Rscript --vanilla brainscore_simulate_vacc.R $SLURM_ARRAY_TASK_ID $n_iter_per_job $iter_total $cor_method $aggre_method $sim_type $gs_type $minGSSize $maxGSSize ""
    else
      ...
      Rscript --vanilla brainscore_simulate_vacc.R $SLURM_ARRAY_TASK_ID $n_iter_per_job $iter_total $cor_method $aggre_method $sim_type $gs_type $minGSSize $maxGSSize $gsScoreList_null
    fi
...
```{r,eval=FALSE}
library(BrainEnrich)
input_dir <- "/gpfs1/home/z/c/zcao4/BrainEnrich/sim_res/GO_MF_spin_brain_pearson_mean_20_200_1000"
output_dir <- "/gpfs1/home/z/c/zcao4/BrainEnrich/sim_res"
save_name <- "GO_MF_spin_brain_pearson_mean_20_200_1000"
tmp <- job_cat(input_dir = input_dir, output_dir = output_dir, n_rds = 1000, delete_originals = TRUE, save_name = save_name)
```

### 5.4. loop through other input arguments
```{bash,eval=FALSE}
#!/bin/bash
#SBATCH --job-name=brainscore_sim    # Job name
#SBATCH --output=sim.out             # Standard output log
...
minGSSize=20              # Minimum gene set size
maxGSSize=200             # Maximum gene set size
sim_types=("spin_brain" "resample_gene")  # Simulation types
gs_types=("SynGO")        # Gene set types
aggre_methods=("median" "meanabs" "maxmean" "ks_weighted" "ks_pos_neg_sum") # Aggregation methods
...
## 5.5. loop through other input arguments to collect the results
```{r,eval=FALSE}
library(BrainEnrich)
...
iter_total <- 1000 # Total number of iterations
cor_method <- "pearson" # Correlation method
aggre_methods <- c("ks_pos_neg_sum", "ks_weighted", "maxmean", "meanabs", "median") # Aggregation methods
null_models <- c("randomize", "resample_gene", "spin_brain") # Null models
