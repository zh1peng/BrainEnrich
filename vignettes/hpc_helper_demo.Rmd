---
title: "How to precompute null gene set scores and do simulation on HPC with slurm"
...
# 2. use job_splitter (a generic function to split jobs) to do brainscore
## 2.1. Prepare the script for the job_splitter function
...
```
## 2.2. Prepare the slurm job file
```{bash,eval=FALSE}
#!/bin/bash
...
## 2.3. Submit the slurm job
put the content of the slurm job file into a file named job_file.slurm and submit the job
```{bash,eval=FALSE}
sbatch job_file.slurm
```

## 2.4. Use job_cat to collect the brainscore results from jobs
```{r,eval=FALSE}
library(BrainEnrich)
...
```
## 2.5. loop through other input arguments
...
# 3. use job_splitter (a generic function to split jobs) to do brainscore.simulate
...
```{r,eval=FALSE}
library(BrainEnrich)
input_dir <- "/gpfs1/home/z/c/zcao4/BrainEnrich/sim_res/SynGO_spin_brain_pearson_mean_20_200_1000"
output_dir <- "/gpfs1/home/z/c/zcao4/BrainEnrich/sim_res"
tmp <- job_cat(input_dir = input_dir, output_dir = output_dir, n_rds = 1000, delete_originals = TRUE)
```
