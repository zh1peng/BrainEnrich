...
null_models <- c("resample_gene", "spin_brain")
gs_types <- c("SynGO", "GO_MF")
aggre_methods <- c("median", "meanabs", "maxmean", "ks_weighted", "ks_pos_neg_sum")
...
## 4. use job_splitter (a generic function to split jobs) to do brainscore.simulate
### 4.1. Prepare the script for the job_splitter function
Here, we will prepare a script for distributed computing environments, allowing large-scale permutation tests to be split across multiple compute nodes. 
```{r,eval=FALSE}
...
### 4.2. Prepare the slurm job file
```{bash,eval=FALSE}
#!/bin/bash
...
### 4.3. Use job_cat to collect the brainscore results from jobs
```{r,eval=FALSE}
library(BrainEnrich)
